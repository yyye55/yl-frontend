    ...
    >
      ...
      >
        ...
        <!--
          ...
          为什么编辑页不该有「暂存」：后端 edit-draft（apps/api/views.py:885 一带）每次进入
          都用**正式 Report 的内容重建草稿**、无条件覆盖。于是编辑页上点「暂存」是这样：
            点 → 弹「暂存成功」→ 状态变「已暂存」→ 用户以为存住了
            → 切走再点一次「修改报名」→ 内容全变回正式报名里的旧内容
          也就是说这个按钮在编辑页给的是**假保证**，比没有按钮更糟。
          （这条是后端侧的问题，已记在《后端协助问题清单-第十二届暂存改造》P1-1；
            后端修好之前，编辑页就别提供这个入口。）
          ...
          用户改的内容确实没上服务器，但他此刻也无事可做（这正是撤掉按钮的原因）。
        -->
        <el-form-item>
          <el-button
            type="primary"
          ...
          <!-- 编辑页不提供「暂存」：后端 edit-draft 每次进入都会用正式报名重建草稿，
               在这里存的草稿下次进来必然被冲掉。详见上方注释第 5 点。 -->
          <el-button
...
async function enterEdit(localCache) {
  try {
    ...
    // 「暂存」按钮在编辑页已被拿掉（后端 edit-draft 每次进入都覆盖草稿，那个按钮给的是
    // 假保证）—— 见模板注释第 5 点。
